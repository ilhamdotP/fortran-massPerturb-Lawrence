...
C      omega bar = OMEGABAR
      ...
      AICORR=1.D0+LBL*GS*MSS*MASST/XA*(1.D0+3.D0*PRESS/EDEN)
     &       *(XA*XA*XA/(MSS*MASST))
     &       /(3.D0*(1.D0-2.D0*GS*MASST*MSS/XA))
     
C     Define things related to derivative of AICORR with rescpect to P

      PAKSEN=-GS*EDEN*MASST*MSS/(XA*XA)
     &        *((1.D0+PRESS/EDEN)
     &       *(1.D0+4.D0*PI*XA*XA*XA*PRESS/(MSS*MASST))
     &       /(1.D0-2.D0*GS*MASST*MSS/XA)
     &       -2.D0/(12.D0*PI)*LBL*(4.D0*PI*XA*XA*XA*EDEN/(MSS*MASST))
     &        *(1.D0+3.D0*PRESS/EDEN)*(1.D0+PRESS/EDEN)
     &       /(1.D0-2.D0*GS*MASST*MSS/XA))
     
      SATU=XA*XA*(1.D0+PRESS/EDEN)*(1.D0+3.D0*PRESS/EDEN)*DFED(PRESS)
     
      DUA=XA*XA*(1.D0+PRESS/EDEN)*EDEN*(3.D0/EDEN-3.D0*PRESS*DFED(PRESS)
     &    /(EDEN**2))
     
      TIGA=XA*XA*(1.D0+3.D0*PRESS/EDEN)*EDEN*(1.D0/EDEN-PRESS
     &     *DFED(PRESS)/(EDEN**2))
    
      EMPAT=2.D0*GS*XA*(1.D0+PRESS/EDEN)*(1.D0+3.D0*PRESS/EDEN)*EDEN
     &      *4.D0*PI*XA*XA*DFED(PRESS)/MSS
     
C      DAICORR is derivative of sigma with respect to pressure
     
      DAICORR=GS*LBL*(SATU+DUA+TIGA+EMPAT)/(3.D0*(1.D0-2.D0
     &        *GS*MASST*MSS/XA))
...
C      solving dm_0/dr; five, six, seven are terms in the eq
      ...
      FIVE=4.D0*PI*DFED(PRESS)*(EDEN+PRESS)*Po*AICORR
     &     /(1.D0-DAICORR)
...
C     KOMPXI is a mathematical thing that is used to find XI 
      ...
      KOMPXI=(-GS*EDEN*MASST*MSS/(XA*XA)
     &        *((1.D0+PRESS/EDEN)
     &       *(1.D0+4.D0*PI*XA*XA*XA*PRESS/(MSS*MASST))
     &       /(1.D0-2.D0*GS*MASST*MSS/XA)
     &       -2.D0/(12.D0*PI)*LBL*(4.D0*PI*XA*XA*XA*EDEN/(MSS*MASST))
...
C     declaring XIE for the deformation of the equator
      ...
      PRINT*, PRESS,EK(J,6),EK(J,7),EK(J,8),EK(J,9)
